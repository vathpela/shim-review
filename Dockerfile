...
RUN wget https://pjones.fedorapeople.org/rhel-7-shim-20210521/repo/redhat-release-server-7.2-9.el7_2.1.x86_64.rpm
...
RUN rpm -e bind-license dbus-glib dbus-python gobject-introspection \
	   ...
	   rootfiles rpm-python vim-enhanced vim-minimal \
	   yum yum-metadata-parser yum-plugin-fastestmirror
RUN wget https://pjones.fedorapeople.org/rhel-7-shim-20210521/shim-15.4-4.el7.src.rpm
RUN rpm -ivh shim-15.4-4.el7.src.rpm
RUN sed -i 's/linux32 -B/linux32/g' /builddir/build/SPECS/shim.spec
RUN rpmbuild -D 'dist .el7' -bb /builddir/build/SPECS/shim.spec
COPY shimia32.efi /
COPY shimx64.efi /
RUN rpm2cpio /builddir/build/RPMS/x86_64/shim-unsigned-ia32-15.4-4.el7.x86_64.rpm  | cpio -diu
RUN rpm2cpio /builddir/build/RPMS/x86_64/shim-unsigned-x64-15.4-4.el7.x86_64.rpm | cpio -diu
